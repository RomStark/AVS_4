...
global  DIRECT_ENCRYPTION
global  CYCLE_ENCRYPTION
global  NUMBER_ENCRYPTION

%include "macros.mac"

section .data
    DIRECT_ENCRYPTION   dd  1
    CYCLE_ENCRYPTION    dd  2
    NUMBER_ENCRYPTION   dd  3
    oneDouble   dq  1.0
...
section .bss
    ...
    ifst        resq    1       ; указатель на файл, открываемый файл для чтения шифрований
    ...
    cont        resb    2120004 ; Массив используемый для хранения данных
...
.next1:
    ...
    jmp .return
.next2:
    ; Генерация случайных шифрований
    mov rdi, [r13+16]
    call atoi
    mov [num], eax
    PrintInt [num], [stdout]
    ...
    xor     rax, rax
    call    srand
    ; Заполнение контейнера случайными шифрованиями
    mov     rdi, cont   ; передача адреса контейнера
    mov     rsi, [num]  ; передача количества шифрований
    call    InRndContainer
    jmp .task2

.next3:
    ; Получение шифрований из файла
    FileOpen [r13+16], "r", ifst
    ; Заполнение контейнера шифрованиями из файла
    mov     rsi, cont           ; адрес контейнера
    mov     rdi, [ifst]         ; указатель на файл
    xor     rax, rax
...
.task2:
    ...
    FileOpen [r13+24], "w", ofst1
    PrintStrLn "Sorted container:", [ofst1]
    PrintContainer cont, [ofst1]
    FileClose [ofst1]
